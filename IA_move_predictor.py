"""
Train a model to predict the next move chosen by the player.

Uses action_chunks (move vs switch dataset) and filters to action_type == "move".
Optionally adds a Bayesian set-dex prior based on observed moves.
"""
    ...
    for input_file in input_files:
        ...
        for example in data:
            if filter_voluntary and not example.get("is_voluntary", True):
                continue
            if example.get("action_type") != "move":
                continue
            seen += 1
...
class FeatureBuilder:
    ...
    def build_example(self, example: Dict[str, object]) -> Optional[Tuple[Dict[str, np.ndarray], np.ndarray]]:
        ...
        move_label = normalize_token(example.get("action_target"))
        label = self.vocab.get(move_label, 0)
...
def build_model(
    ...
    output = tf.keras.layers.Dense(vocab_size + 1, activation="softmax", name="move_id")(x)
...
def train(args: argparse.Namespace) -> None:
    ...
    callbacks = [
        tf.keras.callbacks.EarlyStopping(
            monitor="val_top5", mode="max", patience=3, restore_best_weights=True
        ),
        tf.keras.callbacks.ReduceLROnPlateau(monitor="val_top5", mode="max", patience=2),
    ]
